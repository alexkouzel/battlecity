    ...
    private boolean checkContact(ImageView owner) {
        ...
        for (Node node : game.gridPane.getChildren()) {
            ...
            if (collided && wall != null) {
                ...
                if (wall == Resource.leaf) {
                    bullet.toBack();
                    game.blackBackground.toBack();
                }
                // put bullet upon water or ice
                if (wall == Resource.water || wall == Resource.ice) {
                    bullet.toFront();
                    return false;
                }
                // if brick is new
                if (wall == Resource.brickWall) {
                    imageView.setImage(Resource.brickWall2);
                }
                // if brick is half broken
                if (wall == Resource.brickWall2) {
                    imageView.setImage(Resource.brickWall3);
                }
                // if brick is almost broken
                if (wall == Resource.brickWall3) {
