...
import com.battlecity.entities.GrayAITank;
import com.battlecity.entities.MainTank;
import com.battlecity.entities.Tank;
...
public class GameController {
    ...
    public void creator(ActionEvent actionEvent) throws Exception {
        ...
        // load fxml scene
        URL location = getClass().getClassLoader().getResource(Resources.CreatorView);
    ...
    public void checkWin() {
        if (!isLaunched) {
            if (tanks.size() == 1 && tanks.get(0) == goldTank && ALSs.size() == 0) {
                hasWinner = true;
                winnerView = new ImageView(new Image(Resources.win));
    ...
    public void gameOver() {
        if (!hasWinner) {
            isRunning = false;
            gameOverView = new ImageView(new Image(Resources.gameOver));
    ...
    private void createALS(double x, double y) {
        ImageView ALS = new ImageView(new Image(Resources.als));
    ...
    void startGame(GridPane gridPane, int[] countTanks) {
        ...
        for (Node node : gridPane.getChildren()) {
            if (node instanceof ImageView) {
                ...
                // handle base block
                if (image == Resources.base) {
                    baseView = newImageView;
                }

                // handle ice block
                if (image == Resources.ice) {
                ...
                if (image != null && image != Resources.leaf && image != Resources.ice) {
    ...
    private void initialize() {
        ...
        goldTank.setCoordinates(153, 603, 0, 43);
        goldTank.setImage(Resources.goldTank);
        ...
        grayTank.setImage(Resources.grayTank);
        paneOfGame.getChildren().add(grayTank.getImageView());
        staticObjects.add(grayTank.getImageView());
        tanks.add(grayTank);
    }
